...
finishedBookingList=[];
      ...
      for(var book in allBookingList)
      {
        if(book['booking_status']=='WAITING'&&book['req_date'].toString().contains(value!))
        {
          waitingBookingList.add(BookingModel.fromSnapshot(book));
        }
        else if(book['booking_status']=='ACCEPTED'&&book['booking_final_date'].toString().contains(value!))
        {
          acceptedBookingList.add(BookingModel.fromSnapshot(book));
        }
    else if(book['booking_status']=='REFUSED'&&book['response_date'].toString().contains(value!))
        {
          refusedBookingList.add(BookingModel.fromSnapshot(book));
        }
    else if(book['booking_status']=='CANCELED'&&book['response_date'].toString().contains(value!))
        {
          canceledBookingList.add(BookingModel.fromSnapshot(book));
        }
    else if(book['booking_status']=='FINISHED'&&book['booking_final_date'].toString().contains(value!))
        {
          finishedBookingList.add(BookingModel.fromSnapshot(book));
        }
  ...
  Future<void> changeBookStatus({int? bookId=0,String? bookStatus="",String? date="",String?notes="" ,String? bookType="كشف",String? searchDate=""}) async
  {
    ...
    if (resultMap["status"] == 1) {
      Fluttertoast.showToast(msg:bookStatus=="ACCEPTED"?"تمت الموافقة بنجاح":bookStatus=="REFUSED"?"تم الرفض بنجاح":bookStatus=="CANCELED"?"تم الالغاء بنجاح":"تم الانتهاء من الكشف");
  ...
  Future<void> addBookingFromClinick({String? patientName="",String?painDesc=""}) async
  {
