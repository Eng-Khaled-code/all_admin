...
class AdminController extends GetxController {
  ...
  late List allCoursesList=[];
  late List courses=[];

  late List places=[];
  late List<PlaceModel> blockList=[];
  late List<PlaceModel> flatList=[];
  late List<PlaceModel> groundList=[];
  late List<PlaceModel> storeList=[];

  late List users=[];
  late RxList<UserModel> doctorUsers=<UserModel>[].obs;
  late RxList<UserModel> middlemanUsers=<UserModel>[].obs;
  late RxList<UserModel> normalUsers=<UserModel>[].obs;
  late RxList<UserModel> ecommerceUsers=<UserModel>[].obs;
  late RxList<UserModel> admins=<UserModel>[].obs;
  late RxList<UserModel> coursesAdmin=<UserModel>[].obs;

  late List persons=[];
  late RxList<MissedModel> waitingMissedList=<MissedModel>[].obs;
  late List<MissedModel> acceptedMissedList=[];
  late List<MissedModel> refusedMissedList=[];
  late RxList<MissedModel> waitingFoundList=<MissedModel>[].obs;
  late List<MissedModel> acceptedFoundList=[];
  late List<MissedModel> refusedFoundList=[];
  late Map<String,dynamic> mainDashboard;
  ...
  void onInit() {
    ...
    operations(moduleName: "category",operationType:_fullAccess?"load full access": "load");
   }
      ...
      }) async
  {
    ...
    String _suggestedImage=await _getSuggestions(operationType: operationType,imageName: countryImageName);

    Map<String, String> postData = _getPostData(bookFile: bookFile,lat:lat,long:long,imageFile: imageFile,moduleName:moduleName, operationType: operationType, id: id, addressOrCategoryDescription: addressOrCategoryDescription,usernameOrCategory: usernameOrCategory,emailOrAdminToken: emailOrAdminToken, passwordOrStopReason: passwordOrStopReason,userType: userType,phoneOrMissedType: phoneOrMissedType,countryImageName:operationType=="add suggestion"?_suggestedImage: countryImageName,userStatusOrClinicStatus: userStatusOrClinicStatus);

    String phpPageName=moduleName=="ecommerce"?"products_admin":moduleName=="courses_list"?"course_admin":moduleName=="middleman"?"middleman_admin":moduleName=="category"?"category_admin":moduleName=="mps"?"mps/mps_admin":moduleName=="books"?"read_books/books_admin":"user_admin";
    ...
    String? userStatusOrClinicStatus=""})
  {
    ...
    else if(moduleName=="books")
    {
        ...
        String _imageName="",_base64image="",_fileName="",_base64File="";
    ...
    {
      _postData={"type":operationType!,"user_id":"$id","user_or_clinic_status":userStatusOrClinicStatus!,"password":passwordOrStopReason!,"phone": phoneOrMissedType!,"country":countryImageName!,"address":addressOrCategoryDescription!,"lat":"$lat","long":"$long","username":usernameOrCategory!,"user_type":userType!,"email":emailOrAdminToken!};
    }
  ...
  void _categorizeLists({String? moduleName}) {
    ...
    moduleName=="category"
        ?
    (selectedCategory =selectedCategory?? categoriesList.first['name'])
        :
    moduleName=="mps"
        ?
    mpsSearch(value:"")
  ...
  void _action({String? moduleName, String? operationType}) {
    ...
    _add=operationType=="add",
        ...
        _update=operationType == "update"||operationType=="update book file"||operationType=="update book image",
    ...
    if(_add&&(moduleName=="users"||moduleName=="books"))
    {
      ...
      _changeStatus ? "تم تغيير الحالة بنجاح" : _changeUserType ?
      "تم تغيير نوع المستخدم بنجاح" : _update ? "تم التعديل بنجاح"
          : _addSuggestion ? "تم إرسال الاقتراحات بنجاح" : "تم الاضافة بنجاح"
      ) : () {};
    }
  }
  ...
  void mpsSearch({String? value}) {
     ...
     for(var item in persons)
     {
       ...
       bool
           ...
           _searchValue=value==""?true:item['last_place'].contains(value);
  ...
  void productsSearch({String? value}){
    ...
    for (var element in allProducts) {
      ...
      bool
      _searchValue=value==""?true:element['product_name'].contains(value);
    ...
    coursesAdmin=<UserModel>[].obs;
      for (var element in users) {
        bool
            ...
            _searchValue=value==""?true:element['username'].contains(value);
  ...
  Future<String> _getSuggestions({String? operationType,String? imageName}) async{
