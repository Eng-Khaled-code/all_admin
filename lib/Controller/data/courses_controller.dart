...
class CoursesController extends GetxController
{
  ...
  courseOperations({String? price="0",String? operationType,
    int?id=0,String? name="",File? image,String? oldImageName="",
    String? desc="",int? categoryId=0,int? status=0})async{
    ...
    multipartFile=MultipartFile(image.path, filename: _imageName);
    }

    Map<String,dynamic> _postData=
    {"price":price!,"status":"$status","type":operationType!,
    "id":"$id","desc":desc!,"name":name!,"file":multipartFile??"",
    ...
    print(errorTranslation(resultMap["message"]));
    }
  ...
  Future<void> videoOperations({String? type,int? courseId=0,int? videoId=0,int? secId=0,String? name="" ,String? desc="",File? video,String? vNameForDelOrUp=""}) async
  {
    ...
    Map<String, String> postData = {
      ...
      "type":type!,
      "desc":desc!,
      "video_file_name":type=="update"&&video==null?vNameForDelOrUp!:_videoFileName,
      "base64video":_base64video,
      "old_video_file_name":type=="update"&&video==null?"no":vNameForDelOrUp!,
      "course_id":"$courseId"
  };
