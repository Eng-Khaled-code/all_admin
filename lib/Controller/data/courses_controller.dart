...
class CoursesController extends GetxController {
  String url = Strings.appRootUrl + "courses/";
  ...
  courseOperations(
    ...
    if (operationType == "add" || operationType == "update course image") {
      ...
      //  multipartFile =
      //    await dio.MultipartFile.fromFile(image.path, filename: _imageName);
    }
    ...
    Map<String, dynamic> resultMap =
        await _mainOperation.postOperation(_postData, url + "course_admin.php");
    if (resultMap["status"] == 1) {
      ...
      if (operationType != "load") {
        ...
        Fluttertoast.showToast(
            msg: operationType == "add"
                ? "تم اضافة الكورس بنجاح"
                : operationType == "update course name"
                    ? "تم تعديل اسم الكورس بنجاح"
                    : operationType == "update course image"
                        ? "تم تعديل غلاف الكورس بنجاح"
                        : operationType == "update course status"
                            ? "تم تعديل حالة عرض الكورس بنجاح"
                            : operationType == "update course desc"
                                ? "تم تعديل شرح الكورس بنجاح"
  ...
  Future<void> discountOperations(
      {String? type,
      int? disId = 0,
      String? desc = "",
      String? name = "",
    ...
    Map<String, dynamic> resultMap = await _mainOperation.postOperation(
        postData, Strings.appRootUrl + "ecommerce/discount.php");
    if (resultMap["status"] == 1) {
              ...
              msg: type == "add"
                  ...
                  : type == "update"
                      ...
                      : type == "delete"
                          ? "تم الحذف بنجاح"
                          : (status == 1
                              ? "تم تفعيل العرض بالفعل"
                              : "تمإيقاف العرض بالفعل"))
          : () {};
    } else {
      Get.snackbar("خطا", errorTranslation(resultMap["message"]));
    }
  ...
  Future<void> sectionOperations(
    ...
    Map<String, dynamic> resultMap =
        await _mainOperation.postOperation(postData, url + "section_ope.php");
    if (resultMap["status"] == 1) {
      type != "load"
          ? Fluttertoast.showToast(
              msg: type == "add"
  ...
  Future<void> videoOperations(
      ...
      String? desc = "",
      File? video,
      String? vNameForDelOrUp = ""}) async {
    isVideoLoading(true);

    String _videoFileName = "", _base64video = "";
    if (type == "add" || (type == "update" && video != null)) {
    ...
    Map<String, String> postData = {
      ...
      "desc": desc!,
      "video_file_name":
          type == "update" && video == null ? vNameForDelOrUp! : _videoFileName,
      "base64video": _base64video,
      "old_video_file_name":
          type == "update" && video == null ? "no" : vNameForDelOrUp!,
      "course_id": "$courseId"
    };

    Map<String, dynamic> resultMap =
        await _mainOperation.postOperation(postData, url + "video_ope.php");
    if (resultMap["status"] == 1) {
      type != "load"
          ? Fluttertoast.showToast(
              msg: type == "add"
  ...
  Future<void> questionOperations(
      {String? type,
      int? quesId = 0,
      int? videoId = 0,
      String? question = "",
    ...
    Map<String, dynamic> resultMap =
        await _mainOperation.postOperation(postData, url + "question.php");
    if (resultMap["status"] == 1) {
      type != "load"
          ? Fluttertoast.showToast(
              msg: type == "add"
