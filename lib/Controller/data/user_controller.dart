...
class UserController extends GetxController{
 ...
 // UserModel? userInformation;
  ...
  Future<void> openMyHomePage({String? email, String? password,String? type,String? userId,String?userType}) async {
  isLoading(true);
  page('splach');
  ...
  print(userType+" "+email+" "+password);
    ...
    Strings.userInformation!.value.userType=="doctor"?workDaysOperations(type:"load"):(){};
      ...
      if(type=="log_in")
      {
        ...
        Get.snackbar("خطأ",errorTranslation(resultMap["message"]),);
...
*/
  ...
  Future<void> updateImage({File? image, String?  addOrUpdatePhoto,}) async {
  ...
  if(type!="load") {
    Get.snackbar("نجاح",type=="add"? "تمت اضافة الهاتف بنجاح":type=="update"?"تم تعديل الهاتف بنجاح":"تم حذف الهاتف بنجاح");
  }
  } else {
  Fluttertoast.showToast(msg: errorTranslation(resultMap["message"]),
  toastLength: Toast.LENGTH_LONG,
  ...
  Future<void> workDaysOperations({String? type,int? dayId=0,String?startTime="",String? endTime="" })async{
    ...
    Map<String, dynamic> resultMap = await _mainOperation.postOperation(
        postData, url + "work_days.php");
    if (resultMap["status"] == 1) {

      userWorkDays=resultMap["data"];
      if(type!="load") {
        Get.snackbar("نجاح",type=="add"? "تمت اضافة الموعد بنجاح":type=="update"?"تم تعديل الموعد بنجاح":"تم حذف الموعد بنجاح");
      }
    } else {
      Fluttertoast.showToast(msg: errorTranslation(resultMap["message"]),
          toastLength: Toast.LENGTH_LONG,
  ...
  Future<void> countryOperations({String? type,String? countryId,String? country})async{
    ...
    if (resultMap["status"] == 1) {
      ...
      if(type!="load") {
        Get.snackbar("نجاح",type=="add"? "تمت اضافة الدولة بنجاح":type=="update"?"تم تعديل الدولة بنجاح":"تم حذف الدولة بنجاح");
      }
    } else {
      Fluttertoast.showToast(msg: errorTranslation(resultMap["message"]),
          toastLength: Toast.LENGTH_LONG,
  ...
  Future<void> updateUserFields({String? key,String? value,String? clinickReason=""})async{
    ...
    if (resultMap["status"] == 1) {
      ...
      if(key=="d_clinick_status"&&value=="0"){
        ...
        Fluttertoast.showToast(msg:
            key == "username" ? "تم تعديل الاسم بنجاح" : key == "address"
                ? "تم تعديل العنوان بنجاح"
                :key == "about_doctor"
                ? "تم تعديل التخصص بنجاح":"تم فتح العيادة بنجاح");
      }} else {
      Fluttertoast.showToast(msg: errorTranslation(resultMap["message"]),
          toastLength: Toast.LENGTH_LONG,
          textColor: primaryColor,
