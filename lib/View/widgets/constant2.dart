...
void getImageFile({Function(File file)? onFileSelected}) async {
  FilePickerResult? result = await FilePicker.platform.pickFiles(
      dialogTitle: "أختر صورة", type: FileType.image, allowCompression: true);
...
void getVideoFile({Function(File file)? onVideoSelected}) async {
  FilePickerResult? result = await FilePicker.platform.pickFiles(
      dialogTitle: "أختر فيديو", type: FileType.video, allowCompression: true);
...
void showReasonDialog(
    ...
    String? initialValue = "",
    String? lable = "السبب ؟",
    Function(String reason)? onPress}) {
      ...
      content: Form(
        ...
        child: TextFormField(
          ...
          decoration: InputDecoration(
              labelText: lable, labelStyle: const TextStyle(fontSize: 12)),
        ),
      ),
      actions: <Widget>[
        TextButton(
