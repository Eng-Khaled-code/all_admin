...
class ProfilePage extends StatelessWidget {
  ...
  _userType=userInformation!.value.userType;
    return  Directionality(
      textDirection: TextDirection.rtl,
        child: Scaffold(
          appBar:customAppbar(title:"تحديث البيانات الشخصية",actions:Container()),
    ...
    return Container(
      ...
      child: Column(
        ...
        Row(
          children: [
            ...
            IconButton(icon:const Icon(Icons.add_ic_call_rounded,color: Colors.white,),onPressed: (){
              myCustomDialog(type: "phone",addOrUpdateOrDelete: "add",fieldValue: "",fieldId: "");
        ...
        Container(
          ...
          height: 30,
          child: _userController.userPhones.isEmpty?CustomText(text:"لا يوجد",color: Colors.white,alignment: Alignment.centerRight,)
              : ListView.builder(
            scrollDirection: Axis.horizontal,
            itemCount: _userController.userPhones.length,
            itemBuilder: (context,position)=>InkWell(
              onTap: (){
                myCustomDialog(type: "phone",addOrUpdateOrDelete: "update",fieldValue: _userController.userPhones[position]["number"],fieldId: _userController.userPhones[position]["phone_id"].toString());
                ...
                child: Row(
                  ...
                  children: [
                   GestureDetector(child:const Icon(Icons.close,color: Colors.red,),
                      onTap: (){
                      myCustomDialog(type: "phone",
                          addOrUpdateOrDelete: "delete",
                          fieldValue: _userController.userPhones[position]["number"],
                          fieldId: _userController.userPhones[position]["phone_id"].toString());
                    },),

                    CustomText(
                      text:_userController.userPhones[position]["number"],
                      color: Colors.white,),
                  ],
                ),),
            ),),
  ...
  Row _workDaysTopRow(BuildContext context){
    return Row(
      children: [
        CustomText(text:"ايام العمل",color: Colors.white,alignment: Alignment.centerRight,),
        const SizedBox(width :15),
        IconButton(icon:const Icon(Icons.calendar_today,color: Colors.white,),onPressed: (){
          _showWorkOperationDialog(context:context,type:"إضافة",from:"",to: "",dayId: 0);
  ...
  Widget _workDaysBody(BuildContext context){
    return
      _userController.isWorkDaysLoading.value
          ?
    ...
    height: 180,
    child:
    _userController.userWorkDays.isEmpty
    ?
    CustomText(text:"لا يوجد",color: Colors.white,alignment: Alignment.centerRight,)
        : ListView.builder(
    scrollDirection: Axis.horizontal,
    ...
    from:  _userController.userWorkDays[position]["start_time"],
    to:  _userController.userWorkDays[position]["end_time"]
    ,itemId:  _userController.userWorkDays[position]["id"],context: context),
    ));
  ...
  Widget _workDaysWidget(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
       _workDaysTopRow(context),
        _workDaysBody(context)
        ],),
    );
  }

  Widget _item({String? day,String? from ,String? to,int? itemId,BuildContext? context}){
    return  Padding(
      ...
      _operationRow(context:context,from:from,to:to,itemId:itemId)
  ...
  Widget _operationRow({BuildContext? context,String? from,String? to,int? itemId}){
    return operationRow(
        ()=>_showWorkOperationDialog(context:context,type:"تحديث",from:from!.substring(0,5),to:to!.substring(0,5),dayId:itemId!),
        ()async{
    Get.back();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              CustomText(text:"التخصص",color: Colors.white,alignment: Alignment.centerRight,),
              const SizedBox(width :15),
  ...
  Widget _nameAddressField({String? nameOrAddress,String? value}) {
    return InkWell(
      onTap: (){
        myCustomDialog(type: nameOrAddress,addOrUpdateOrDelete: "update",fieldValue:value,fieldId:"");
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context,position){
                ...
                if(position<_userController.userCountries.length){
                  selectColor= countries.contains(_userController.userCountries[position]["country"])
                      ?
                  Colors.blueGrey
                      :
                  Colors.transparent;
                ...
                return InkWell(onTap: (){
                  if(_userController.userCountries.length>1&&selectColor!=Colors.blueGrey){
                    Fluttertoast.showToast(msg: "لا يمكنك استهداف اكثر من دولتين");
                  }
                  else if(_userController.userCountries.length==1&&selectColor==Colors.blueGrey){
                  ...
                  else
                    {
                      selectColor==Colors.blueGrey
                          ?
                       _userController.countryOperations(type: "delete",countryId: _userController.userCountries[position]["country_id"].toString(),country: "")
                       :
                      _userController.countryOperations(type: "add",countryId:"",country: countries[position]);
                    }
  ...
  void _showWorkOperationDialog({BuildContext? context,String? type,String? from,String? to ,int? dayId}) {
...
Get.back();                      await _userController.workDaysOperations(
  ...
  void _showDatePacker(TextEditingController timeController,BuildContext context) async{
