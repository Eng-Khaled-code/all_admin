...
class _HomePageState extends State<HomePage> {
            ...
            child: Scaffold(
              ...
              floatingActionButton: _userType == "full_access"
                  ...
                  : _userType == "course_admin" && currentIndex == 0
                      ? _floatingActionButtonForCourse()
                      : null,
            )));
  }
  ...
  _appBar() {
    return AppBar(
      title: CustomText(
        text: _mainAppbarTitle,
        fontWeight: FontWeight.bold,
        fontSize: 16,
        alignment:
            _userType == "admin" ? Alignment.centerRight : Alignment.center,
      ),
  ...
  FloatingActionButton? _floatingActionButton() {
    bool _user = currentIndex == 1,
        _book = currentIndex == 2,
        _category = currentIndex == 3;
    return currentIndex == 0
        ? null
        : FloatingActionButton.extended(
            onPressed: () => _category
                ? operationDialog(
                    type: "إضافة قسم لل" + Strings.categoryType,
                    controller: _adminController)
                : Get.to(_user ? const AddUser() : const AddBook()),
            icon: Icon(_user
                ? Icons.person_add
                : _book
                    ? CupertinoIcons.book
      ...
      onPressed: () {
        if (courseCategory == null) {
          Fluttertoast.showToast(
              msg: "لا توجد اقسام للكورسات حاليا يرجي ابلاغ المسئول");
        } else {
  ...
  Widget _drawerButton() => Container(
      margin: const EdgeInsets.all(8.0),
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        border: Border.all(color: primaryColor),
