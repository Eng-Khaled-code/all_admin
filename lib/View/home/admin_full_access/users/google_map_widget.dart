...
class GoogleMapsWidget extends StatefulWidget {
  ...
  const GoogleMapsWidget(
      {Key? key, this.lat, this.long, this.markerTitle, this.type})
      : super(key: key);
...
class _GoogleMapsWidgetState extends State<GoogleMapsWidget> {
  ...
  Widget build(BuildContext context) {
    if (widget.lat == null || widget.long == null) {
      ...
      _markers.add(getCustomMarker(
          lat: widget.lat!, long: widget.long!, title: widget.markerTitle));
      setState(() {});
    }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: customAppbar(
            title: widget.type == "show_map" ? "العنوان" : "تحديد العنوان",
            actions: Container()),
  ...
  Marker getCustomMarker({double? lat, double? long, String? title}) {
    return Marker(
        ...
        draggable: widget.type == "get_address" ? true : false,
        onDragEnd: (LatLng latLng) {
          if (widget.type == "get_address") {
            _adminController.setMapData(
                lat: latLng.latitude, long: latLng.longitude, address: title);
          }
        });
  }
}
