...
class CategoryCard extends StatelessWidget {
  ...
  const CategoryCard({Key? key, this.model, this.adminController})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => operationDialog(
          type: "تعديل القسم",
          id: model!.id,
          name: model!.name,
    ...
    return InkWell(
      onTap: () {
        if (model!.itemsCount == "0") {
          showDialogFor(
              contentText: (model!.status == 0
                  ? "هل بالفعل تريد تفعيل هذا القسم"
                  : "هل بالفعل تريد ايقاف هذا القسم"),
              title: "تغيير",
              onPress: () async {
                Get.back();
                await adminController!.operations(
                    ...
                    userStatusOrClinicStatus: model!.status == 0 ? "1" : "0");
              });
        } else {
          Fluttertoast.showToast(
              msg: "لا يمكنك ايقاف هذا القسم لانه يحتوي علي منتجات");
        }
      },
  ...
  Padding _dataWidget() {
    String details =
        "يحتوي علي ${model!.itemsCount}${Strings.categoryType == "تسوق" ? " منتج" : " كورس"} \n الشرح : ${model!.description}";
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          model!.status == 1
              ? Container()
              : dateWidget(model!.changeStatusDate!, "status_changed")
        ],
      ),
    );
  }
