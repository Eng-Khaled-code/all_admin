...
class BookCard extends StatelessWidget {
  ...
  const BookCard({Key? key, this.model, this.adminController})
      : super(key: key);
      ...
      children: [
        Padding(
          ...
          child: InkWell(
            ...
            child: CustomText(
                text: model!.name!.substring(0, _length > 30 ? 30 : _length),
                color: Colors.black,
        ...
        Row(
          children: [
            likeWidget(
                counter: model!.likeCount.toString(),
                icon: Icons.favorite,
                iconColor: Colors.pink),
            likeWidget(
                counter: model!.blackCount.toString(),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          image: DecorationImage(
            image:
                NetworkImage(Strings.bookImageDirectoryUrl + model!.imageUrl!),
            fit: BoxFit.cover,
              ...
              if (choice == "تغيير الكتاب") {
                getBookFile(onFileSelected: (file) async {
                  await adminController!.operations(
                    ...
                    id: model!.id,
                    bookFile: file,
                    phoneOrMissedType: model!.bookUrl!.split('/').last,
                  );
                });
              } else {
                getImageFile(onFileSelected: (File file) async {
                  adminController!.operations(
                    moduleName: "books",
                    operationType: "update book image",
                    id: model!.id,
                    imageFile: file,
                    addressOrCategoryDescription:
