...
class CourseCard extends StatefulWidget {
  ...
  final String? screenType;
   CourseCard({Key? key, this.model, this.coursesController,this.screenType="user",this.adminController})
      : super(key: key);
...
class _CourseCardState extends State<CourseCard> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: (){
        Get.to(CourseDetails(coursesController:widget.coursesController,courseModel: widget.model));
      },
      child: Card(
  ...
  GestureDetector _courseImage() {
    return
      GestureDetector(
        onTapDown: (details) => widget.screenType=="user"?_showPopUpMenue(details):(){},
        child: Container(
          height: Get.height * .2,
          width: Get.width * .3,
 ...
 InkWell _priceContainer(){
  ...
  Widget _stopWidget() {
    return widget.model
    !.mainAdminStatus==0?CustomText(text: "تم إيقاف العرض لان "+widget.model!.mainAdminStopReason!,color: Colors.red,maxLine: 3,alignment: Alignment.topRight,textAlign: TextAlign.start,
    )
    :
    Container
    (
  ...
  InkWell _courseTitle() {
    return InkWell(
      onTap: () {
        if(widget.screenType=="user")
        {
            showReasonDialog(title: "تغيير اسم الكورس", initialValue: widget.model!.name,
            lable: "اسم الكورس", onPress: (name) {
              Get.back();
              widget.coursesController
              !.courseOperations(operationType:"update course name",
              name:name,id:widget.model!.id!);
  ...
  Row _bottomRow() {
    return Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
    likeWidget(counter: widget.model!.likeCount.toString
        (),icon: Icons.favorite,iconColor: Colors.pink),
    likeWidget(counter: widget.model!.blackCount.toString(),icon: Icons.disabled_by_default,iconColor: Colors.grey),
    likeWidget(counter: widget.model!.usersCount.toString(),icon: Icons.person,iconColor: Colors.green),
  ...
  _showPopUpMenue(TapDownDetails details) {
    final List<String> _updateTypes = ["عرض الكورس", "تغيير الصورة"];
    showMenu(
        context: Get.context!,
            ...
            onTap: () {
              if (choice == "تغيير الصورة") {
                getImageFile(onFileSelected: (file) async {
                  ...
                  !.courseOperations(operationType:"update course image",
                  id:widget.model!.id,image:file,oldImageName:widget.model!.imageUrl!.split('/').last,);
  ...
  _discountWidget() {
    ...
    GestureDetector(
            onTapDown: (details) =>    widget.screenType=="user"?setState(()=>
  ...
  operationType: "delete",
   ...
   _topRowWidget() {
    ...
    return  showDiscountRow? Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
    widget.model!.discountStatus==0
    ?
        TextButton.icon(onPressed: ()=>_showAddUpdateDiscountDialog("إختيار عرض"), icon:const Icon(Icons.add_rounded,size:20,color: Colors.blue,)
      , label: CustomText(text:"إضافة عرض",fontSize: 12,))
          :
      Row(children:[
      TextButton.icon(onPressed: ()=>_showAddUpdateDiscountDialog("إختيار عرض اخر"), icon:const Icon(Icons.edit,color: Colors.blue,size: 20,) ,
      label: CustomText(text:"إختيار عرض اخر",fontSize: 12)),
      TextButton.icon(onPressed: ()=>showDialogFor(contentText:"هل بالفعل تريد إلغاء العرض  حيث ان نسبةالخصم تساوي   : "+widget.model!.descountPercentage!+" % ",title: "إلغاء العرض",
      onPress: ()async{
      Get.back();
      await widget.coursesController!.addOrUpdateDeleteCourseDiscount(
      discountId: widget.model!.discountId,type: "delete",courseId:widget.model!.id);
      }), icon:const Icon(Icons.close,color: Colors.blue,size: 20,)
      , label: CustomText(text:"إيقاف العرض",fontSize: 12))]),

      ],):Container();
    }

  IconButton _deleteWidget() {
    return IconButton(onPressed: ()=>_onPressDelete(),
  ...
  IconButton _showWidget() {
    return IconButton(onPressed: (){ _showUnshowDialog();},
        icon: Icon(
          widget.model!.status==1?Icons.visibility:Icons.visibility_off, color: widget.model!
  ...
  IconButton _showWidgetForAdmin() {
    return IconButton(onPressed: ()=>
      widget.model!.mainAdminStatus==1?_showReasonDialog():_showAcceptDialog(),
        icon: Icon(
          widget.model!.mainAdminStatus ==1?Icons.visibility:Icons.visibility_off, color: widget.model!
              .mainAdminStatus==1?Colors.green:Colors.grey,));

  }

  void _showReasonDialog() {
    showReasonDialog(title:"هل تريد إيقاف عرض هذا الكورس",onPress: (comingReason)async{
      Navigator.pop(context);
  ...
  void _showUnshowDialog() {
    bool show = widget.model
    !.status==1?false:true;
    showDialogFor(
    title: "تأكيد",
    contentText:show ?"هل تريد عرض هذا الكورس":"هل تريد إيقاف عرض هذا الكورس",
    onPress: ()async{
    Get.back();
    await widget.coursesController!.courseOperations(operationType:"update course status",status: show?1:0 ,id: widget.model!.id);
  ...
  Container _discountEmptyCart() {
    return Container(
      ...
      child: Column(children: [
        CustomText(
          text: "لا توجد عروض", fontSize: 14, fontWeight: FontWeight.bold,),
        CustomText(text: "يمكنك إضافة عروض من خلال شاشة العروض والخصومات",
            color: Colors.grey,
            maxLine: 3)
  ...
  Widget _discountCard(int position, String type) {
    return widget.coursesController
    !.discountList[position]['status']==0?Container():
    InkWell(onTap: ()async{
    Get.back();
    await widget.coursesController!.addOrUpdateDeleteCourseDiscount(
    discountId: widget.coursesController!.discountList[position]['id'],type: type=="إختيار عرض"?"add":"update",courseId:widget.model!.id);
    ...
    CustomText(text:widget.coursesController!.discountList[position]['name']!+"  نسبة الخصم : "
    +widget.coursesController!.discountList[position]['discount_percentage'].toString(),
    fontSize: 14,fontWeight: FontWeight.bold,),
    dateWidget(widget.coursesController!.discountList[position]['end_in']!,"end"),
  ...
  void _showAddUpdateDiscountDialog(String type){
    ...
    Get.dialog( Directionality(
        ...
        child: AlertDialog(
          ...
          title: CustomText(text:type,fontSize: 20,fontWeight: FontWeight.bold,),
          actions: [
            TextButton(onPressed: ()=>Get.back(), child:const Text("إلغاء"))
          ],

          content: SizedBox(
            height:widget.coursesController!.discountList.isEmpty||_checkEmpty()?120: 300,
            child: widget.coursesController!.discountList.isEmpty||_checkEmpty()?_discountEmptyCart():ListView.builder(
