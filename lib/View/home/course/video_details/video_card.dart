...
class VideoCard extends StatefulWidget {
  ...
  const VideoCard(
      {Key? key, this.model, this.coursesController, this.courseId = 0})
      : super(key: key);
...
class _VideoCardState extends State<VideoCard> {
  ...
  void initState() {
    ...
    String videoUrl = Strings.coursesVideosDirectoryUrl +
        widget.courseId.toString() +
        "/" +
        widget.model!.secId.toString() +
        "/" +
    ...
    return InkWell(
      onTap: () {
        widget.coursesController!
            .questionOperations(type: "load", videoId: widget.model!.id);
        Get.to(() => VideoWatchingPage(
              model: widget.model,
      ...
      child: Card(
        ...
        child:
            Padding(padding: const EdgeInsets.all(10), child: _dataRowWidget()),
      ),
    );
  }
  ...
  Row _dataRowWidget() {
    return Row(
        ...
        children: [_videoImage(), const SizedBox(width: 5), _dataColumn()]);
  }
  ...
  Directionality _dateWidget() {
    String date = widget.model!.createdAt!;
