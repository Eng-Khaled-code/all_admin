...
class VideosPage extends StatelessWidget {
  ...
  VideosPage({Key? key, this.sectionModel,this.coursesController}) : super(key: key);
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body:RefreshIndicator(
          onRefresh: ()async{
        await coursesController!.videoOperations(type: "load",secId: sectionModel!.id);
        },
      child:_dataWidget()      ),
    ));
  }
  ...
  AppBar _myAppar(){
    return customAppbar(title: sectionModel!.name,actions:
    likeWidget(counter: sectionModel!.videoCount.toString(),icon: Icons.video_library,iconColor: Colors.orange),
    );
  }
  ...
  Widget _dataWidget() {
    ...
    return Obx(
            ()=>coursesController!.isVideoLoading.value
                ?
            loadingWidget()
                :
            coursesController!.videoList.isEmpty?noDataCard(text:  "لا توجد فيديوهات",icon: Icons.video_library):
            ...
            ListView.builder(
              ...
              shrinkWrap: true,
                ...
                itemBuilder: (context,position){
                  VideoModel _model=VideoModel.fromSnapshot(coursesController!.videoList[position]);
                  return VideoCard(model: _model,coursesController: coursesController,courseId: sectionModel!.courseId);
                }
