    ...
    return InkWell(
      ...
      child: Card(child: ListTile(trailing: operationRow((){
        ...
        showReasonDialog(title: "تعديل سكشن",initialValue: sectionModel!.name,lable: "السكشن",onPress: (secName)async{
          Get.back();
          await coursesController!.sectionOperations(type: "update",name: secName,secId: sectionModel!.id,courseId: sectionModel!.courseId);
        });
        ...
        {
          Fluttertoast.showToast(msg: "لا يمكنك حذف هذا السكشن لانه يحتوي علي فيديوهات");
        }
        else
          {
                Navigator.pop(context);
                await coursesController!.sectionOperations(
                    type: "delete", courseId: sectionModel!.courseId,
                    secId: sectionModel!.id
                );
        }  },true),title:CustomText(text:sectionModel!.name!,color: Colors.black,alignment: Alignment.topRight,),subtitle: Text(sectionModel!.videoCount!+" فيديو \n "+sectionModel!.createdAt!.substring(0,10)),)),
    );
  }
}
