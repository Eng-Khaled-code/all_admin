...
class MissedCard extends StatelessWidget {
  ...
  _topRow(BuildContext context) {
    return Row(
        ...
        children:[
          ...
          adminProfileWidget(name: model!.username,image:model!.userImage ,date:model!.createdAt),
    model!.missedStatus=="انتظار"?_missedActionStatus():model!.missedStatus=="مقبول"&&model!.missedOrFound=="فقد"?_suggestWidget():Container()
    ],
    );
  }
  ...
  Row _missedActionStatus() {
    return Row(children: [
      IconButton(onPressed: ()=>_showAcceptDialog(),icon: const Icon(Icons.check,color: Colors.blue,),),
    IconButton(onPressed: ()=>_showReasonDialog(),icon: const Icon(Icons.close,color: Colors.red,),)
    ],);

  }

  void _showReasonDialog() {
    String _missedOrFoundEnglish=model!.missedOrFound=="فقد"?"missed":"found";
    showReasonDialog(title:"هل تريد رفض الطلب",onPress: (comingReason)async{
       Get.back();
  ...
  IconButton _suggestWidget() {
    return IconButton(onPressed: ()=>showDialogFor(title: "بحث",contentText: "هل تريد بدأ البحث عن هذا الشخص",onPress: ()async{
      Get.back();
      await adminController!.operations(
      moduleName: "mps",
      id: model!.id,operationType: "add suggestion",countryImageName: model!.image!.split("/").last
      );
