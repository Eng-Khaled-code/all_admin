...
class DayOperationPage extends StatelessWidget {
    ...
    return doctorController.firstBookingIndex.value == -1
        ...
        : TopBookingCard(
            ...
            model: doctorController
                .acceptedBookingList[doctorController.firstBookingIndex.value],
            index: doctorController.firstBookingIndex.value,
            listLength: doctorController.acceptedBookingList.length,
  ...
  _secondBookingIndex() {
    return doctorController.secondBookingIndex.value == -1 ||
            doctorController.secondBookingIndex >=
                doctorController.acceptedBookingList.length
        ? Container()
        : TopBookingCard(
            number: 2,
            model: doctorController
                .acceptedBookingList[doctorController.secondBookingIndex.value],
            index: doctorController.secondBookingIndex.value,
            listLength: doctorController.acceptedBookingList.length,
            doctorController: doctorController);
  }
  ...
  _nextBookingLabel(String title) {
    return Column(
      children: [
        const Divider(color: Colors.white,height: 2,thickness: 1),
        Padding(
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, position) {
                      BookingModel model = type == "accepted"
                          ...
                          : type == "waiting"
                              ? doctorController.waitingBookingList[position]
                              : type == "refused"
                                  ? doctorController
                                      .refusedBookingList[position]
                                  : type == "canceled"
                                      ? doctorController
                                          .canceledBookingList[position]
                                      : doctorController
                                          .finishedBookingList[position];

                      return BottomBookingCard(
                        model: model,
                        doctorController: doctorController,
                      );
                    })));
            ...
            TextButton(
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    Get.back();
                    doctorController.addBookingFromClinick(
                        patientName: _patientNameCon.text,
                        painDesc: _painDescCon.text);
                  }
                },
                child: const Text("إضافة", style: TextStyle(fontSize: 13))),
            TextButton(
                onPressed: () {
                  Get.back();
                },
  ...
  _addBookingButton() {
    return Padding(
      ...
      child: CustomButton(
        ...
        text: "إضافة حجز",
        color: const [Colors.blue, Colors.blue],
