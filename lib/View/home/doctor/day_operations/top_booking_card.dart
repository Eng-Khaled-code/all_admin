...
class TopBookingCard extends StatelessWidget {
  ...
  TopBookingCard(
      {Key? key, this.model, this.number, this.index, this.listLength, this.doctorController})
      :super(key: key);
  ...
  Widget build(BuildContext context) {
    thisDay = date.year.toString() + "-" + date.month.toString() + "-" +
        date.day.toString();
  ...
  Padding columnData() {
    return Padding(
      ...
      child: Column(
        ...
        CustomText(text: model!.patientName!, color: primaryColor, fontSize: 18,
          fontWeight: FontWeight.bold, alignment: Alignment.center,),
        CustomText(text: model!.painDesc!, color: Colors.grey,
  ...
  void showFinishDialogWithNoDate() {
    Get.dialog(CustomAlertDialog(
      ...
      onPress: () async {
        ...
        !.changeBookStatus(bookId:model!.id,bookStatus:"FINISHED",date:model!.finalBookDate,bookType: "إعادة",notes:"يرجي تقييم الدكتور",searchDate:thisDay);
            ...
            child: AlertDialog(
              ...
              actions: <Widget>[TextButton(
                  onPressed: () async {
                    ...
                    await doctorController!.changeBookStatus(bookId:model!.id,bookStatus:"ACCEPTED",date:_dateController.text,bookType:"إعادة",notes:"يرجي تقييم الدكتور",searchDate:thisDay);
  ...
  void _showDatePacker(TextEditingController dateController,
  ...
  Row _cancelBookingWidget(BuildContext context) {
    return Row(mainAxisAlignment: MainAxisAlignment.center,
        children: [const Text(
            "لإلغاء الطلب إضغط ", style: TextStyle(color: Colors.black))
        , GestureDetector(
  ...
  Padding _finishBookingWidget(BuildContext context) {
    return Padding(
      ...
      child: CustomButton(color: const [
        primaryColor,
        Color(0xFF0D47A1),
      ],
          text: model!.bookType=="كشف"?"إنهاء و تحديد موعد الإعادة":"إنهاء",
          onPress: () {
            ...
            GestureDetector(
              ...
              const Text(" التالي ",
                  style: TextStyle(fontWeight: FontWeight.bold,
                      ...
                      decoration: TextDecoration.underline))
              , onTap: () {
              if (listLength! > index!+1 && index!+1 != doctorController!.firstBookingIndex.value) {
                doctorController
              !.setSecondBookingIndex(index! +1);
              }
              else if(listLength! > index!+1 && (index!+1).obs == doctorController!.firstBookingIndex)
              {
  ...
  Widget _enteringClinicRoomWidget() {
    ...
    !.firstBookingIndex.value ==-1? Padding(
    padding: const EdgeInsets.all(20.0),
    child: CustomButton(color: const [
    primaryColor,
    Color(0xFF0D47A1),
  ...
  void _showClinicDialog() {
    ...
    Get.dialog( CustomAlertDialog(
      title: "تنبيه",
      onPress: (){
        int? second=doctorController!.acceptedBookingList.length > (index!+1)?index! +1:(index!)>0?index!-1:-1;
